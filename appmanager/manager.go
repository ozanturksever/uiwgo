...
func (am *AppManager) Initialize(ctx context.Context) error { // ctx reserved for future use
    ...
    // Setup bridge manager: respect existing (e.g., mock) manager; do not create real manager in tests
    if bridge.GetManager() != nil {
        am.bridge = bridge.GetManager()
    } else {
        logutil.Log("Bridge manager not set; continuing without bridge")
    }
...
func (am *AppManager) AddHook(event LifecycleEvent, hook LifecycleHook) { am.lifecycle.AddHook(event, hook) }
...
func (am *AppManager) Mount(root func() g.Node) error {
    ...
    if am.config.EnableRouter && len(am.config.Routes) > 0 && am.bridge != nil {
        // Prefer #router-outlet, fallback to mount element
        var outlet bridge.DOMElement
        if el, err := bridge.GetElementByID("router-outlet"); err == nil && el != nil {
            outlet = el
        } else if el2, err2 := bridge.GetElementByID(am.config.MountElementID); err2 == nil && el2 != nil {
            outlet = el2
        }
        if outlet != nil {
